...
//AVISOS DE MENSAJE
const smsAvisoRG = () => { return `PROYECTO X EBG\n\n` } 
const smsAvisoAG = () => { return `*INFORMACIÓN*\n\n` }
const smsAvisoIIG = () => { return `*INFORMACIÓN*\n\n` }
const smsAvisoFG = () => { return `*ERROR*\n\n` }
const smsAvisoMG = () => { return `*LO USASTE MAL PANA*\n\n` }
const smsAvisoEEG = () => { return `*REPORTE*\n\n` }
const smsAvisoEG = () => { return `PROYECTO X EBG\n\n` }

//PARÁMETROS EN COMANDOS
const smsRowner = () => { return `\`\`\`» 𝙀𝙎𝙏𝙀 𝘾𝙊𝙈𝘼𝙉𝘿𝙊 𝙎𝙊𝙇𝙊 𝙈𝙄 𝘾𝙍𝙀𝘼𝘿𝙊𝙍 LO USAAA.\`\`\`` }//NUMERO DE BOT
const smsOwner = () => { return `\`\`\`» 𝙀𝙎𝙏𝙀 𝘾𝙊𝙈𝘼𝙉𝘿𝙊 𝙎𝙊𝙇𝙊 𝙈𝙄 𝘾𝙍𝙀𝘼𝘿𝙊𝙍 LO USAAA.\`\`\`` }//OWNER
...
const smsAdmin = () => { return `\`\`\`» 𝙀𝙎𝙏𝙀 𝘾𝙊𝙈𝘼𝙉𝘿𝙊 𝙀𝙎 𝙋𝘼𝙍𝘼 𝘼𝘿𝙈𝙄𝙉𝙄𝙎𝙏𝙍𝘼𝘿𝙊𝙍𝙀𝙎.\`\`\`` }//ADMINS
const smsBotAdmin = () => { return `\`\`\`» 𝙉𝙀𝘾𝙀𝙎𝙄𝙏𝙊 𝙎𝙀𝙍 𝘼𝘿𝙈𝙄 𝙋𝘼𝙍𝘼 𝙌𝙐𝙀 𝙋𝙐𝙀𝘿𝘼𝙎 𝙐𝙎𝘼𝙍 𝙀𝙎𝙏𝙀 𝘾𝙊𝙈𝘼𝙉𝘿𝙊\`\`\`` }//BOT CON ADMIN
...
const smsLista1 = () => { return `🌟 INFORMACIÓN ÉLITE BOT🌟`}
...
*╚══════════════*`}
const smsBye = () => { return `ADIOS BASURA.
@user* 
`}
...
//_allantilink.js
const smsTextoYT = () => { return ' ELITE BOT '} 
...
const smsCont19 = () => { return `𝙈𝙎𝙅 𝙀𝙇𝙄𝙈𝙄𝙉𝘼𝘿𝙊`}
const smsCont20 = () => { return `┃👤 𝙐𝙨𝙪𝙖𝙧𝙞𝙤:`}
const smsCont21 = () => { return `┃💠 𝘼 𝙘𝙤𝙣𝙩𝙞𝙣𝙪𝙖𝙘𝙞𝙤𝙣 𝙨𝙚 𝙢𝙤𝙨𝙩𝙧𝙖𝙧𝙖́ 𝙚𝙡 𝙢𝙚𝙣𝙨𝙖𝙟𝙚 𝙚𝙡𝙞𝙢𝙞𝙣𝙖𝙙𝙤 𝙙𝙚 𝙚𝙨𝙩𝙖 𝙥𝙚𝙧𝙨𝙤𝙣𝙖 ...`}
...
𝙋𝘼𝙍𝘼 𝙈𝘼𝙎 𝙄𝙉𝙁𝙊 𝘾𝙊𝙉𝙏𝘼́𝘾𝙏𝘼𝙏𝙀 𝘾𝙊𝙉 𝙈𝙄 𝘾𝙍𝙀𝘼𝘿𝙊𝙍.
+593979133620`}

//_antilink.js
const smsAdwa = () => { return `${global.lenguajeGB['smsAvisoEG']()}𝘾𝙊𝙈𝙊 𝙀𝙍𝙀𝙎 𝘼𝘿𝙈𝙄𝙉 𝙀𝙉 𝙀𝙇 𝙂𝙍𝙐𝙋𝙊 𝙉𝙊 𝙎𝙀𝙍𝘼𝙎 𝙀𝙇𝙄𝙈𝙄𝙉𝘼𝘿𝙊(𝘼)`}
const smsEnlaceWat = () => { return `${lenguajeGB['smsAvisoAG']()}𝑺𝑬 𝑫𝑬𝑻𝑬𝑪𝑻𝑶́ 𝑼𝑵 𝑬𝑵𝑳𝑨𝑪𝑬 𝑵𝑶 𝑷𝑬𝑹𝑴𝑰𝑻𝑰𝑫𝑶 𝑬𝑵 𝑬𝑺𝑻𝑬 𝑮𝑹𝑼𝑷𝑶.\n\n𝑷𝑹𝑶𝑪𝑬𝑫𝑶 𝑨 𝑬𝑳𝑰𝑴𝑰𝑵𝑨𝑹 𝑻𝑼 𝑴𝑬𝑵𝑺𝑨𝑱𝑬 𝒀 𝑨 𝑬𝑳𝑰𝑴𝑰𝑵𝑨𝑹𝑻𝑬 𝑴𝑨𝑳𝑫𝑰𝑻𝑨 𝑬𝑺𝑪𝑶𝑹𝑰𝑨 𝑨𝑵𝑫𝑨 𝑯𝑨𝑪𝑬𝑹 𝑷𝑼𝑩𝑳𝑰𝑪𝑰𝑫𝑨𝑫 𝑬𝑵 𝑶𝑻𝑹𝑶 𝑳𝑨𝑫𝑶.👊🏻`}

//_antilink2.js
const smsEnlaceWatt = () => { return `${lenguajeGB['smsAvisoAG']()}𝙎𝙀 𝘿𝙀𝙏𝙀𝘾𝙏𝙊 𝙐𝙉 𝙀𝙉𝙇𝘼𝘾𝙀 𝙋𝙍𝙊𝙃𝙄𝘽𝙄𝘿𝙊 𝙌𝙐𝙀 𝘾𝙊𝙉𝙏𝙄𝙀𝙉𝙀 𝙃𝙏𝙏𝙋𝙎\n\n𝑷𝑹𝑶𝑪𝑬𝑫𝑶 𝑨 𝑬𝑳𝑰𝑴𝑰𝑵𝑨𝑹 𝑻𝑼 𝑴𝑬𝑵𝑺𝑨𝑱𝑬 𝒀 𝑨 𝑬𝑳𝑰𝑴𝑰𝑵𝑨𝑹𝑻𝑬 𝑴𝑨𝑳𝑫𝑰𝑻𝑶 𝑩𝑨𝑺𝑻𝑨𝑹𝑫𝑶 𝑯𝑨𝑪𝑬𝑹 𝑷𝑼𝑩𝑳𝑰𝑪𝑰𝑫𝑨𝑫 𝑬𝑵 𝑶𝑻𝑹𝑶 𝑳𝑨𝑫𝑶.👊🏻`}
...
const smsModP2 = () => { return `Habilitar función para que todos puedan usar ÉLITE BOT`}
const smsModAd1 = () => { return `🛂 MODO ADMIN`}
const smsModAd2 = () => { return `Solo los Admins podrán usar ÉLITE BOT en Grupos`}
...
const smsMens4 = () => { return `𝘉𝘖𝘛`} 
const smsMens5 = () => { return `𝘎𝘙𝘜𝘗𝘖`} 
...
//_antiviewonce.js
const smsAntiView = () => { return `*YO TODO LO VEO HUMANO*`} 
...
//_expired.js
const smsBottem1 = () => { return `‼️ 𝘘𝘶𝘦𝘳𝘪𝘥𝘰 𝘩𝘶𝘮𝘢𝘯𝘰 𝘴𝘦 𝘵𝘦 𝘵𝘦𝘳𝘮𝘪𝘯𝘰́ 𝘦𝘭 𝘵𝘪𝘦𝘮𝘱𝘰 𝘨𝘳𝘢𝘵𝘪𝘴 𝘥𝘦 𝘱𝘳𝘶𝘦𝘣𝘢 𝘥𝘦 𝘌𝘭𝘪𝘵𝘦 𝘉𝘰𝘵, 𝘱𝘢𝘳𝘢 𝘳𝘦𝘯𝘰𝘷𝘢𝘳 𝘦𝘭 𝘴𝘦𝘳𝘷𝘪𝘤𝘪𝘰 𝘤𝘰𝘯𝘵𝘢́𝘤𝘵𝘢𝘵𝘦 𝘤𝘰𝘯 𝘮𝘪 𝘤𝘳𝘦𝘢𝘥𝘰𝘳.

👤 https://Wa.me/593993370003
Síguenos en Instagram:
» https://www.instagram.com/elitebotglobal?igsh=a2MyeXRpOXJndWlx`} 
...
const smsMalused3 = () => { return `¿A quien doy admin?`}
...
//grupo-tagall.js
const smstagaa = () => { return `*ACTÍVENSEN PANAS 👋🏻*`}
